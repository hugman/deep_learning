...
def sequence_loss_by_example(logits, targets, weights, num_decoder_symbols,
  ...
  with tf.op_scope(logits + targets + weights, name,
    ...
    for i in xrange(len(logits)):
      if softmax_loss_function is None:
        ...
        indices = targets[i] + num_decoder_symbols * tf.range(0, batch_size, 1)
        with tf.device("/cpu:0"):  # Sparse-to-dense must happen on CPU for now.
          dense = tf.sparse_to_dense(indices, tf.expand_dims(length, 0), 1.0, 0.0)
        target = tf.reshape(dense, [-1, num_decoder_symbols])
        crossent = tf.nn.softmax_cross_entropy_with_logits( logits[i], target, name="SequenceLoss/CrossEntropy{0}".format(i))
      else:
  ...
  return log_perps # 전체 time step 에 대해서, 전체 batch 대한 대표 loss 값이 나온다. 
